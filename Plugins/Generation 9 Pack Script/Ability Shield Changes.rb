...
class Battle::Battler
  ...
  def pbUseMove(choice, specialUsage = false)
    ...
    if user.hasActiveAbility?([:LIBERO, :PROTEAN]) &&
       ...
       user.pbHasOtherType?(move.calcType) && !GameData::Type.get(move.calcType).pseudo_type && !@battle.isBattlerActivedAbility?(user)
      @battle.pbShowAbilitySplash(user)
      user.pbChangeTypes(move.calcType)
      typeName = GameData::Type.get(move.calcType).name
      @battle.pbDisplay(_INTL("{1}'s type changed to {2}!", user.pbThis, typeName))
      @battle.pbHideAbilitySplash(user)
      @battle.setBattlerActivedAbility(user)
    ...
    if @lastMoveUsed && !self.lastMoveFailed
      ...
      if move.recoilMove? && defined?(move.pbRecoilDamage)
        targets.each{|target|
