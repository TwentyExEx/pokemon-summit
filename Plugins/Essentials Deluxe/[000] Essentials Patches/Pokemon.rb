...
#-------------------------------------------------------------------------------
def pbNumOwnedSpecies(species, attribute = nil, value = nil)
  count = 0
  $player.pokemon_party.each { |p| count += 1 if p&.isSpecies?(species) && p.check_for_attribute(attribute, value) }
  pbEachNonEggPokemon { |p| count += 1 if p&.isSpecies?(species) && p.check_for_attribute(attribute, value) }
  return count
end
