...
def pbApplyWildMarks(pokemon)
  ...
  # Always apply Mini/Jumbo Marks on wild Pokemon who meet the size requirements.
  if Settings::GUARANTEED_WILD_SIZE_MARKS
    case pokemon.scale
    when 0   then pokemon.giveMemento(:MINIMARK)
    when 255 then pokemon.giveMemento(:JUMBOMARK)
    end
  end
end
