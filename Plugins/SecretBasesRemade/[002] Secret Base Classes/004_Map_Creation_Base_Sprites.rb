...
class SecretBaseOwnerSprite
  def initialize(event, map, viewport = nil)
    ...
    @base_id        = $PokemonMap.current_base_id
    @active_base    = SecretBaseMethods.is_active_secret_base?(@base_id)
    @player_base    = SecretBaseMethods.is_player_base?(@base_id)
    update_graphic
  end
  ...
  def update_graphic
    if !@player_base && @active_base
      trainer_type = SecretBaseMethods.get_base_from_id(@base_id).owner.trainer_type
      filename = GameData::TrainerType.charset_filename_brief(trainer_type)
      if filename
        @event.character_name = filename
      else
...
module SecretBaseMethods
  def self.load_baseline_map(base_location, template_data,map)
    ...
    if template_data.owner_location
      event=RPG::Event.new(template_data.owner_location[0],template_data.owner_location[1])
      key_id = (map.events.keys.max || 0) + 1
      event.name = "BaseOwner"
      event.id = key_id
      Compiler::push_script(event.pages[0].list,"pbSecretBaseOwner")
      Compiler::push_end(event.pages[0].list)
      map.events[key_id] = event
    end
  end
...
EventHandlers.add(:on_new_spriteset_map, :add_secret_base_exit_graphics,
  proc { |spriteset, viewport|
    map = spriteset.map
    next if map.map_id == SecretBaseSettings::SECRET_BASE_MAP
...
EventHandlers.add(:on_new_spriteset_map, :add_secret_base_pc_graphics,
  proc { |spriteset, viewport|
    ...
    map.events.each do |event|
      if event[1].name[/SecretPC/i]
        spriteset.addUserSprite(SecretBasePCSprite.new(event[1], map, viewport))
      elsif event[1].name[/BaseOwner/i]
        spriteset.addUserSprite(SecretBaseOwnerSprite.new(event[1], map, viewport))
      end
    end
  }
)
